...
describe('Release', () => {
  ...
  it('should set action url with release id', () => {
    ...
    expect(baseElement.querySelector('form')).toHaveAttribute('action', '/repo/repo/releases/edit/1')
  })

  it('should set action url without release id', () => {
    const { baseElement } = render(<Release {...props} release={undefined} />)
    expect(baseElement.querySelector('form')).toHaveAttribute('action', '/repo/repo/releases/edit')
  })
  ...
  it('should set delete release button link', () => {
    render(<Release {...props} />)
    expect(screen.getByText('Delete')).toHaveAttribute('href', '/repo/repo/releases/delete/1')
  })
