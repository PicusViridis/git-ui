...
import { deleteUser, getUsers, postUser, Req1, Req2 } from '../../../src/controllers/users'
...
describe('postUser', () => {
  it('should create user', async () => {
    jest.spyOn(prisma.user, 'create').mockResolvedValue(mockUser1)
    const req = getMockReq<Req1>({ body: { username: 'username', password: 'password' } })
    const { res } = getMockRes()
...
describe('deleteUser', () => {
  it('should get users', async () => {
    ...
    const req = getMockReq<Req2>({ params: { id: '1' } })
    const { res } = getMockRes()
